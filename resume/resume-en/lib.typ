...
#let justified-header(primary, secondary) = {
  ...
  pad[
    #grid(
      rows: (auto, 0em), // 两行布局
      columns: (1fr,auto), // 第二列自动宽度
      [
        #pad[#primary] // primary在第一行
        #align(right)[#secondary-right-header[#secondary]] // secondary在第二行右侧
      ]
    )
  ]
}
...
#let resume(
  ...
  show heading.where(level: 2): it => {
    set text(color-darkgray, size: 11pt, style: "normal", weight: "regular")
    it.body
  }
  ...
  let contacts = {
    ...
    align(left)[
      #set text(size: 12pt, weight: "light", style: "normal")
      #block[
        #align(horizon)[
  ...
  let letter-heading = {
    grid(columns: (1fr, 2fr), 
      ...
      align(right+horizon)[
        #block(clip: true, stroke: 0pt, radius: 0cm, 
        width: 4cm, height: 5cm, profile_picture)
      ],
    )
  }
...
#let resume-gpa(numerator, denominator, rank, rank_all, sigle, english) = {
  set text(size: 11pt, style: "normal", weight: "light")
  [
  #text[\ Grade point average (rank):#box[#strong[#numerator]/#denominator (#strong[#rank]/#rank_all)]]
  #text[#h(12em)]
  #text[English level:#english]
  #linebreak()
  #text[Major Courses:#sigle]
  ]
}
